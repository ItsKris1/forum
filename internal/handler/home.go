...
func Home(env *env.Env) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		homePage := HomePage{
			...
			AllPosts:           posts,
			AllTags:            tags,
			SuccessfulRegister: oauth.UserRegistered, // If user was redirected to home page after successful register
		}

		// Reset the value because we displayed the message after user was redirected to home page after registration
		// Otherwise the message would appear all the time on the home page
		oauth.UserRegistered = false
