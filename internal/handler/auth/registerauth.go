...
func RegisterAuth(env *env.Env) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if query.RowExists("SELECT username FROM users WHERE username = ?", username, db) { // if the username exists
			RegMsgs.TakenUn = true
			invalidInput = true
		}

		if query.RowExists("SELECT email from USERS WHERE email = ?", email, db) { // if the email exists
